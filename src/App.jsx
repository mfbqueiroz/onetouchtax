...
// ── IMPROVED PDF PARSER ────────────────────────────────────────────────────
// Handles multiple bank statement formats: BOA, Chase, Wells Fargo, etc.
function parseBankText(text) {
  ...
  const patterns = [
    // MM/DD/YY or MM/DD/YYYY + description + amount (positive or negative)
    /^(\d{1,2}\/\d{1,2}\/\d{2,4})\s+(.{3,80?}?)\s+([-–]?\$?[\d,]+\.\d{2})(?:\s|$)/,
    // Date at start, amount with optional $ and negative
    /(\d{1,2}\/\d{1,2}\/\d{2,4})\s+(.+?)\s+([-–]?\$?[\d,]+\.\d{2})$/,
    // Chase format: MM/DD desc amount
    /^(\d{2}\/\d{2})\s+(.{4,70}?)\s{2,}([-–]?\$?[\d,]+\.\d{2})/,
    // BOA format with transaction type
    /^(\d{2}\/\d{2}\/\d{4})\s+([A-Z#].*?)\s+([-–]?\$?[\d,]+\.\d{2})\s*([-–]?\$?[\d,]+\.\d{2})?/,
    // Wells Fargo: date desc credit/debit balance
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const pat of patterns) {
      ...
      // Amount: take first match, strip $ and commas
      ...
      if (parts.length === 3 && parts[2].length === 2) {
        ...
        date = `${parts[0]}/${parts[1]}/${curYear}`;
      }

      txns.push({
        id: `p${i}-${Math.random().toString(36).slice(2)}`,
        date,
        description: desc,
        amount: amt,
        aiCategory: null,
    ...
    // If no pattern matched, try a looser approach: find any date + number on the same line
    if (!matched) {
      ...
      if (looseDate && looseAmt) {
        ...
        if (!isNaN(amt) && Math.abs(amt) > 0.01) {
          ...
          const desc = beforeAmt || afterDate.slice(0, 60);
          if (desc.length > 2) {
...
function exportPDF(txns, entityType, userName, company) {
  ...
  const rows = txns.map(t => `
    <tr style="border-bottom:1px solid #e2e8f0">
      ...
      <td style="padding:6px 8px;font-size:11px;text-align:right;color:${t.amount>=0?"#10b981":"#334155"}">${t.amount>=0?"+":"–"}$${Math.abs(t.amount).toLocaleString("en-US",{minimumFractionDigits:2})}</td>
  ...
  h1{font-size:22px;margin:0}h2{font-size:14px;margin:0 0 4px}
  ...
  .box{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px 20px;flex:1}
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #0a2a5e;padding-bottom:16px;margin-bottom:20px">
    <div><h1>OneTouch Tax — Transaction Report</h1><p style="color:#64748b;font-size:12px;margin:4px 0 0">${company} · ${cfg.label} (${cfg.form})</p></div>
  ...
  <div class="summary">
    ...
    <div class="box"><div class="box-label">Deductible Expenses</div><div class="box-val" style="color:#1055b8">$${expense.toLocaleString("en-US",{minimumFractionDigits:2})}</div></div>
    <div class="box"><div class="box-label">Net Income (est.)</div><div class="box-val" style="color:#0a2a5e">$${(income-expense).toLocaleString("en-US",{minimumFractionDigits:2})}</div></div>
    <div class="box"><div class="box-label">Total Transactions</div><div class="box-val" style="color:#8b5cf6">${txns.length}</div></div>
  </div>
...
function exportQBO(txns, entityType, company) {
  // QuickBooks IIF format (importable into QuickBooks Desktop & Online via journal entry)
  const lines = [
  ...
  txns.forEach((t, i) => {
    const acct = t.aiCategory || "Uncategorized";
    const memo = t.description.replace(/\t/g, " ");
  ...
  a.href = url;
  a.download = `onetouchtax_${new Date().toISOString().slice(0,10)}.iif`;
  a.click();
...
const CatBadge = ({ label, entityType }) => {
  const s = TYPE_META[getCatType(label, entityType)];
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#f0f7ff,#e8f4ff)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Manrope',system-ui", padding: 24 }}>
      ...
      <div style={{ width: "100%", maxWidth: 560, background: "#fff", borderRadius: 24, border: "1px solid #e2e8f0", boxShadow: `0 24px 64px ${BRAND.touchColor}14`, overflow: "hidden" }}>
        <div style={{ background: BRAND.gradient, padding: "24px 32px", display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div><div style={{ color: "#fff", fontWeight: 800, fontSize: 18 }}>Configuração inicial</div><div style={{ color: "rgba(255,255,255,.7)", fontSize: 13, marginTop: 2 }}>2 passos para liberar o acesso</div></div>
        </div>
      ...
      <div style={{ marginLeft: 212, flex: 1, padding: "28px 28px 48px" }}>
        ...
        {tab === "home" && <div>
          <div style={{ marginBottom: 22 }}>
            <h1 style={{ fontFamily: "'Manrope',system-ui", fontSize: 28, fontWeight: 800, color: "#0f172a", margin: "0 0 4px", letterSpacing: "-1px" }}>Olá, {user.name?.split(" ")[0]} 👋</h1>
          ...
          {txns.length === 0 ? (
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: "40px 24px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 13, color: "#64748b", marginBottom: 20 }}>Importe seu primeiro extrato para começar.</div>
        ...
        {tab === "transactions" && <div>
          ...
          {txns.length === 0 ? (
            ...
            <div className="card" style={{ padding: 0, overflow: "hidden" }}>
              ...
              {txns.map(t => (
                <div key={t.id} style={{ display: "grid", gridTemplateColumns: "88px 1fr 108px 170px 80px 110px", gap: 10, alignItems: "center", padding: "11px 16px", borderBottom: "1px solid #f8fafc", background: t.status === "approved" ? "#f0fdf4" : t.status === "rejected" ? "#fef2f2" : "#fff" }}>
                  ...
                  <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
                    {t.status === "approved" ? <span style={{ fontSize: 10, fontWeight: 700, color: "#10b981" }}>✓ Ok</span>
                      : t.status === "rejected" ? <span style={{ fontSize: 10, fontWeight: 700, color: "#ef4444" }}>✗ Rev.</span>
                      : (<>
                        <button className="ab gn" onClick={() => approveT(t.id)} title="Confirmar">✓</button>
                        <button className="ab rd" onClick={() => rejectT(t.id)} title="Rejeitar">✕</button>
                      </>)}
                  </div>
                </div>
              ))}
          ...
          <div style={{ marginTop: 10, fontSize: 11, color: "#94a3b8", display: "flex", gap: 16 }}>
            <span>✓ Confirmadas: <strong>{approved}</strong></span>
            <span>⏳ Pendentes: <strong>{pending}</strong></span>
            <span style={{ color: "#94a3b8" }}>Confirme as transações antes de exportar.</span>
          </div>
        </div>}

        {/* EXPORT — PDF and QuickBooks only */}
        {tab === "export" && <div>
          ...
          {txns.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              ...
              <div className="card" style={{ borderTop: `3px solid #ef4444` }}>
                ...
                <h3 style={{ fontFamily: "'Manrope',system-ui", fontSize: 18, fontWeight: 800, color: "#0f172a", marginBottom: 8 }}>Relatório PDF</h3>
                <p style={{ color: "#64748b", fontSize: 13, lineHeight: 1.6, marginBottom: 16 }}>Relatório completo com todas as transações classificadas, resumo de receitas e deduções. Ideal para arquivamento e envio ao contador.</p>
                <ul style={{ listStyle: "none", marginBottom: 20, display: "flex", flexDirection: "column", gap: 6 }}>
                  {["✓ Todas as transações listadas", "✓ Resumo de receitas e despesas", "✓ Categorias IRS incluídas", "✓ Pronto para imprimir"].map(f => <li key={f} style={{ fontSize: 12, color: "#475569" }}>{f}</li>)}
                </ul>
                <div style={{ marginBottom: 12, padding: "10px 14px", background: "#f8fafc", borderRadius: 10, fontSize: 12, color: "#64748b" }}>
                  <strong>{txns.length}</strong> transações · <strong>{approved}</strong> confirmadas · <strong>{pending}</strong> pendentes
                </div>
                <button onClick={() => exportPDF(txns, entityType, user.name, user.company)} style={{ width: "100%", background: "#ef4444", color: "#fff", border: "none", borderRadius: 12, padding: "13px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                  📄 Exportar PDF →
                </button>
              </div>
              ...
              <div className="card" style={{ borderTop: `3px solid #10b981` }}>
                <div style={{ fontSize: 40, marginBottom: 14 }}>🏦</div>
                <h3 style={{ fontFamily: "'Manrope',system-ui", fontSize: 18, fontWeight: 800, color: "#0f172a", marginBottom: 8 }}>QuickBooks (.IIF)</h3>
                <p style={{ color: "#64748b", fontSize: 13, lineHeight: 1.6, marginBottom: 16 }}>Arquivo no formato IIF compatível com QuickBooks Desktop e Online. Importe diretamente como lançamentos contábeis.</p>
                <ul style={{ listStyle: "none", marginBottom: 20, display: "flex", flexDirection: "column", gap: 6 }}>
                  {["✓ Formato IIF (QuickBooks nativo)", "✓ Lançamentos por categoria IRS", "✓ Compatível com QB Desktop & Online", "✓ Importação direta sem ajustes"].map(f => <li key={f} style={{ fontSize: 12, color: "#475569" }}>{f}</li>)}
                </ul>
                <div style={{ marginBottom: 12, padding: "10px 14px", background: "#f8fafc", borderRadius: 10, fontSize: 12, color: "#64748b" }}>
                  <strong>{txns.length}</strong> transações · Empresa: <strong>{user.company || "—"}</strong>
                </div>
                <button onClick={() => exportQBO(txns, entityType, user.company)} style={{ width: "100%", background: "#10b981", color: "#fff", border: "none", borderRadius: 12, padding: "13px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                  🏦 Exportar QuickBooks (.IIF) →
                </button>
              </div>
            </div>
          )}
          <div style={{ marginTop: 16, padding: "12px 16px", background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: 10, fontSize: 12, color: "#92400e" }}>
            💡 <strong>Dica:</strong> Confirme as transações na aba Transações antes de exportar para garantir que as classificações estejam corretas.
          </div>
        </div>}
    ...
    <div style={{ minHeight: "100vh", background: "#f8faff", fontFamily: "'Manrope',system-ui" }}>
      ...
      <div style={{ padding: "24px 24px 48px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div><h1 style={{ fontFamily: "'Manrope',system-ui", fontSize: 22, fontWeight: 800, color: "#0f172a", margin: "0 0 3px", letterSpacing: "-0.8px" }}>Validação de Transações</h1><div style={{ fontSize: 12, color: "#64748b" }}>{cfg.label} · {cfg.form} · Clique na categoria para editar</div></div>
        ...
        <div style={{ marginTop: 10, fontSize: 11, color: "#94a3b8", display: "flex", gap: 20 }}><span>🟢 &gt;85%</span><span>🟡 60–85%</span><span>🔴 &lt;60% revisar</span><span>💡 Clique na categoria para editar</span></div>
      </div>
    </div>
  );
};
...
const Login = () => {
  ...
  const go = async () => {
    ...
    catch (e) { setErr(e.code === "auth/invalid-credential" || e.code === "auth/wrong-password" || e.code === "auth/user-not-found" ? "E-mail ou senha inválidos." : "Erro ao entrar. Tente novamente."); setLoading(false); }
  };
