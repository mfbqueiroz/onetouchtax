...
function buildMonthlySummary(txns, entityType) {
  const data = {}; // { category: { 0..11: amount } }
  txns.forEach(t => {
...
function exportSummaryExcel(txns, entityType, company, businessDesc) {
  ...
  const addCatRows = (catList, groupLabel) => {
    catList.forEach(cat => {
      const row = ["", groupLabel, cat];
      let total = 0;
      activeMths.forEach(m => {
        const v = data[cat]?.[m] || 0;
        total += v;
        row.push(v ? +v.toFixed(2) : null);
      });
      row.push(total ? +total.toFixed(2) : null);
      rows.push(row);
    });
  };

  rows.push(["","",""]);
  addCatRows(cats.income, "Income");
  ...
  activeMths.forEach(m => {
    let s = 0;
    cats.income.forEach(c => { s += data[c]?.[m] || 0; });
    grandInc += s;
    totalIncomeRow.push(s ? +s.toFixed(2) : null);
  });
  totalIncomeRow.push(+grandInc.toFixed(2));
  rows.push(totalIncomeRow);
  rows.push(["","",""]);

  addCatRows(cats.expenses, "Expenses");
  ...
  activeMths.forEach(m => {
    let s = 0;
    cats.expenses.forEach(c => { s += Math.abs(data[c]?.[m] || 0); });
    grandExp += s;
    totalExpRow.push(s ? +s.toFixed(2) : null);
  });
  ...
  activeMths.forEach(m => {
    let inc = 0, exp = 0;
    cats.income.forEach(c => { inc += data[c]?.[m] || 0; });
    cats.expenses.forEach(c => { exp += Math.abs(data[c]?.[m] || 0); });
    plRow.push(+(inc-exp).toFixed(2));
  });
  plRow.push(+(grandInc-grandExp).toFixed(2));
  rows.push(plRow);
  rows.push(["","",""]);
  rows.push(["","",company || ""]);
  ...
  XLSX.utils.book_append_sheet(wb, ws, "English");
...
const MonthlySummaryTab = ({ txns, entityType, user }) => {
  ...
  const mthSum = (catList, m) => catList.reduce((s,c) => s + Math.abs(data[c]?.[m]||0), 0);
  const mthInc = m => cats.income.reduce((s,c) => s + (data[c]?.[m]||0), 0);
  const mthExp = m => mthSum(cats.expenses, m);
  ...
  const sectionHdr = (label,color) => (
    <tr><td colSpan={activeMths.length+2} style={{padding:"10px 12px 4px",fontSize:10,fontWeight:800,textTransform:"uppercase",letterSpacing:1,color,background:"#f8fafc",borderTop:"2px solid #e2e8f0"}}>{label}</td></tr>
  );
  const totalRow = (label,mthFn,total,color,bg="#f0f4ff") => (
    <tr style={{background:bg}}>
      <td style={{padding:"9px 12px",fontSize:12,fontWeight:800,color,paddingLeft:16}}>{label}</td>
      {activeMths.map(m => { const v=mthFn(m); return <td key={m} style={{...tdStyle,fontWeight:700,color,background:bg}}>{v?fmt(v):"—"}</td>; })}
      <td style={{...tdStyle,fontWeight:800,color,background:bg}}>{fmt(total)}</td>
    </tr>
  );
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18}}>
        <div>
          ...
          <div style={{fontSize:12,color:"#64748b"}}>{user.company} · {cfg.label} · {cfg.form}{user.businessDescription&&` · ${user.businessDescription}`}</div>
        </div>
        <button onClick={()=>exportSummaryExcel(txns,entityType,user.company,user.businessDescription)} style={{background:"#10b981",color:"#fff",border:"none",borderRadius:10,padding:"9px 18px",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>📊 Exportar Excel</button>
      </div>
      ...
      <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,overflow:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",minWidth:600}}>
          <thead>
            <tr>
              <th style={{...thStyle,textAlign:"left",minWidth:200}}>Categoria</th>
              {activeMths.map(m=><th key={m} style={thStyle}>{MONTH_NAMES[m]}</th>)}
              <th style={{...thStyle,color:BRAND.arcOuter}}>TOTAL</th>
            </tr>
          </thead>
          <tbody>
            {sectionHdr("📈 Receitas","#15803d")}
            {cats.income.map(cat => {
              ...
              return (
                <tr key={cat}>
                  ...
                  {activeMths.map(m => { const v=data[cat]?.[m]||0; return <td key={m} style={v?tdStyle:tdZero}>{v?fmt(v):"—"}</td>; })}
                  <td style={{...tdStyle,fontWeight:700,color:"#15803d"}}>{tot?fmt(tot):"—"}</td>
                </tr>
              );
            })}
            {totalRow("Total Receitas", mthInc, grandInc, "#15803d", "#f0fdf4")}

            {sectionHdr("💼 Deduções",BRAND.touchColor)}
            {cats.expenses.map(cat => {
              ...
              return (
                <tr key={cat}>
                  ...
                  <td style={{...tdStyle,fontWeight:700,color:BRAND.touchColor}}>{tot?fmt(tot):"—"}</td>
                </tr>
              );
            })}
            ...
            <tr style={{background: (grandInc-grandExp)>=0?"#f0fdf4":"#fef2f2"}}>
              ...
              {activeMths.map(m => {
                const pl = mthInc(m) - mthExp(m);
                return <td key={m} style={{padding:"11px 12px",fontSize:12,fontWeight:700,textAlign:"right",color:pl>=0?"#15803d":"#dc2626",borderTop:"2px solid #e2e8f0",background:(grandInc-grandExp)>=0?"#f0fdf4":"#fef2f2"}}>{fmtPL(pl)}</td>;
              })}
      ...
      <div style={{marginTop:12,fontSize:11,color:"#94a3b8"}}>
        💡 Apenas transações confirmadas ou pendentes são incluídas. Transações rejeitadas são excluídas.
      </div>
    </div>
  );
};
...
async function classifyBatch(batch, entityType, company, businessDesc) {
  ...
  const bizContext = businessDesc ? `Business: "${company}" — Industry/Activity: "${businessDesc}". Use this context to improve classification accuracy (e.g., gasoline for a car rental = fleet expense, not personal).` : `Business: "${company}".`}`;
  ...
  try {
    ...
    if(data.error){console.error("classify error:",data.error);return[];}
...
const OneTouchZone=({onDone,entityType,user})=>{
...
const ClientApp=({user,onLogout})=>{
  ...
  useEffect(()=>{
    ...
    const t=setTimeout(async()=>{
      try{
        await updateDoc(doc(db,"users",user.uid),{transactions:txns,entityType,txnUpdatedAt:new Date().toISOString()});
    ...
    <div style={{minHeight:"100vh",background:"#f8faff",fontFamily:"'Manrope',system-ui",display:"flex"}}>
      ...
      <div style={{width:212,background:"#fff",borderRight:"1px solid #e2e8f0",padding:"20px 12px",display:"flex",flexDirection:"column",position:"fixed",top:0,bottom:0,left:0,zIndex:10}}>
        ...
        <div style={{borderTop:"1px solid #f1f5f9",paddingTop:12}}>
          ...
          {saveStatus==="saving"&&<div style={{textAlign:"center",fontSize:10,color:"#94a3b8",padding:"4px 0"}}>💾 Salvando...</div>}
          {saveStatus==="saved"&&<div style={{textAlign:"center",fontSize:10,color:"#10b981",padding:"4px 0",fontWeight:700}}>✓ Salvo</div>}
          <VersionBadge/>
        </div>
      </div>
      <div style={{marginLeft:212,flex:1,padding:"28px 28px 48px"}}>
        {tab==="home"&&<div>
          <div style={{marginBottom:22}}><h1 style={{fontFamily:"'Manrope',system-ui",fontSize:28,fontWeight:800,color:"#0f172a",margin:"0 0 4px",letterSpacing:"-1px"}}>Olá, {user.name?.split(" ")[0]} 👋</h1><p style={{color:"#64748b",fontSize:13,margin:0}}>{user.company} · {cfg.form}{user.businessDescription&&` · ${user.businessDescription}`}</p></div>
        ...
        {tab==="transactions"&&<div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              {txns.length>0&&<button onClick={()=>{if(confirm(`Apagar todas as ${txns.length} transações?`))setTxns([]);}} style={{background:"#fef2f2",color:"#dc2626",border:"1px solid #fca5a5",borderRadius:10,padding:"8px 14px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>🗑 Limpar</button>}
        ...
        {tab==="export"&&<div>
          ...
          :(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:20}}>
            <div className="card" style={{borderTop:"3px solid #ef4444"}}><div style={{fontSize:40,marginBottom:14}}>📄</div><h3 style={{fontFamily:"'Manrope',system-ui",fontSize:18,fontWeight:800,color:"#0f172a",marginBottom:8}}>Relatório PDF</h3><p style={{color:"#64748b",fontSize:13,lineHeight:1.6,marginBottom:16}}>Listagem completa de todas as transações. Ideal para envio ao contador.</p><button onClick={()=>exportPDF(txns,entityType,user.name,user.company)} style={{width:"100%",background:"#ef4444",color:"#fff",border:"none",borderRadius:12,padding:"13px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>📄 Exportar PDF →</button></div>
            <div className="card" style={{borderTop:"3px solid #10b981"}}><div style={{fontSize:40,marginBottom:14}}>🏦</div><h3 style={{fontFamily:"'Manrope',system-ui",fontSize:18,fontWeight:800,color:"#0f172a",marginBottom:8}}>QuickBooks (.IIF)</h3><p style={{color:"#64748b",fontSize:13,lineHeight:1.6,marginBottom:16}}>Lançamentos por categoria IRS. Importação direta no QuickBooks.</p><button onClick={()=>exportQBO(txns,entityType,user.company)} style={{width:"100%",background:"#10b981",color:"#fff",border:"none",borderRadius:12,padding:"13px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>🏦 Exportar QuickBooks (.IIF) →</button></div>
            <div className="card" style={{borderTop:"3px solid #8b5cf6"}}><div style={{fontSize:40,marginBottom:14}}>📊</div><h3 style={{fontFamily:"'Manrope',system-ui",fontSize:18,fontWeight:800,color:"#0f172a",marginBottom:8}}>Resumo Mensal (.xlsx)</h3><p style={{color:"#64748b",fontSize:13,lineHeight:1.6,marginBottom:16}}>Planilha auxiliar com receitas/despesas mês a mês, igual à planilha do contador.</p><button onClick={()=>exportSummaryExcel(txns,entityType,user.company,user.businessDescription)} style={{width:"100%",background:"#8b5cf6",color:"#fff",border:"none",borderRadius:12,padding:"13px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>📊 Exportar Resumo Mensal →</button></div>
          </div>)}
        ...
        {tab==="settings"&&<div>
          ...
          <div className="card"><h3 style={{fontSize:14,fontWeight:700,color:"#0f172a",marginBottom:12}}>👤 Perfil</h3>{[["Nome",user.name],["E-mail",user.email],["Empresa",user.company],["Ramo de Atividade",user.businessDescription||"—"],["Banco",user.bank||"—"],["Conta",user.account||"—"]].map(([k,v])=><div key={k} style={{display:"flex",justifyContent:"space-between",padding:"9px 0",borderBottom:"1px solid #f1f5f9",fontSize:13}}><span style={{color:"#64748b"}}>{k}</span><span style={{fontWeight:600,color:"#0f172a"}}>{v}</span></div>)}</div>
        </div>}
      </div>
    </div>
  );
      ...
      <div style={{padding:"28px 28px 60px",maxWidth:1000,margin:"0 auto"}}>
        ...
        {showForm&&<div style={{background:"#fff",border:`1.5px solid ${BRAND.touchColor}`,borderRadius:16,padding:24,marginBottom:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
            ...
            <div><label style={{fontSize:11,fontWeight:700,color:"#64748b",display:"block",marginBottom:5}}>Ramo de Atividade</label><input className="inp" placeholder="Ex: Aluguel de carros, Limpeza, TI..." value={form.businessDescription} onChange={e=>setForm(f=>({...f,businessDescription:e.target.value}))}/></div>
