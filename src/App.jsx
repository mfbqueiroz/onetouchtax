...
const MonthlySummaryTab = ({ txns, entityType, user }) => {
  ...
  const totalRow = (label,mthFn,total,color,bg) => (
...
async function classifyBatch(batch, entityType, company, businessDesc) {
  ...
  const bizContext = businessDesc
    ? `Business: "${company}" - Industry/Activity: "${businessDesc}". Use this context to improve classification (e.g. gasoline for a car rental = fleet expense, not personal).`
    : `Business: "${company}".`;
    ...
    <div style={{minHeight:"100vh",background:"#f8faff",fontFamily:"'Manrope',system-ui",display:"flex"}}>
      ...
      <div style={{width:212,background:"#fff",borderRight:"1px solid #e2e8f0",padding:"20px 12px",display:"flex",flexDirection:"column",position:"fixed",top:0,bottom:0,left:0,zIndex:10}}>
        ...
        <div style={{background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:9,padding:"7px 10px",marginBottom:16}}>
          ...
          {user.businessDescription&&<div style={{fontSize:10,color:"#64748b",marginTop:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>🏢 {user.businessDescription}</div>}
        </div>
      ...
      <div style={{padding:"28px 28px 60px",maxWidth:1000,margin:"0 auto"}}>
        ...
        <div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,overflow:"hidden"}}>
          ...
          {users.map(u=><div key={u.uid} style={{display:"grid",gridTemplateColumns:"1fr 1fr 100px 90px 80px",gap:12,padding:"13px 16px",borderBottom:"1px solid #f8fafc",alignItems:"center",background:u.active===false?"#fffbeb":"#fff"}}>
            ...
            <div><div style={{fontSize:12,color:"#475569",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{u.company||"—"}</div>{u.businessDescription&&<div style={{fontSize:10,color:"#94a3b8",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>🏢 {u.businessDescription}</div>}</div>
