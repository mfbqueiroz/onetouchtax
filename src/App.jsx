...
function parseBankText(text) {
  ...
  lines.forEach((line,i)=>{
    ...
    if(m){ const amt=parseFloat(m[3].replace(/[,–]/g,"")); if(!isNaN(amt)&&Math.abs(amt)>.01) txns.push({id:`p${i}-${Math.random()}`,date:m[1],description:m[2].trim(),amount:amt,aiCategory:null,status:"unclassified"}); }
  });
  return txns;
}
  ...
  return new Promise(resolve=>{
    ...
    reader.onload=async(e)=>{
      try{
        ...
        let full="";
        for(let p=1;p<=pdf.numPages;p++){const pg=await pdf.getPage(p);const ct=await pg.getTextContent();full+=ct.items.map(i=>i.str).join(" ")+"\n";}
    ...
    <div style={{minHeight:"100vh",background:"#f8faff",fontFamily:"'Manrope',system-ui",display:"flex"}}>
      ...
      <div style={{width:212,background:"#fff",borderRight:"1px solid #e2e8f0",padding:"20px 12px",display:"flex",flexDirection:"column",position:"fixed",top:0,bottom:0,left:0,zIndex:10}}>
        ...
        <nav style={{display:"flex",flexDirection:"column",gap:2,flex:1}}>
          {NAV.map(n=><button key={n.id} className={`ntb ${tab===n.id?"a":""}`} onClick={()=>setTab(n.id)}>{n.icon} {n.label}{n.id==="import"&&<span style={{marginLeft:"auto",background:BRAND.touchColor,color:"#fff",fontSize:9,fontWeight:800,padding:"1px 7px",borderRadius:10}}>1 toque</span>}</button>)}
        </nav>
...
const Login=()=>{
  ...
  const go=async()=>{
    ...
    try{ await signInWithEmailAndPassword(auth,email,pass); }
      ...
      setErr(e.code==="auth/invalid-credential"||e.code==="auth/wrong-password"||e.code==="auth/user-not-found"?"E-mail ou senha inválidos.":"Erro ao entrar. Tente novamente.");
      setLoading(false);
    }
  };
      ...
      <div style={{width:420,padding:48,background:"#fff",borderRadius:24,border:"1px solid #e2e8f0",boxShadow:`0 24px 64px ${BRAND.touchColor}14`}}>
        ...
        {resetSent?(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <button onClick={go} disabled={loading} style={{background:loading?"#94a3b8":BRAND.gradient,color:"#fff",padding:14,borderRadius:10,border:"none",fontFamily:"inherit",fontSize:15,fontWeight:700,cursor:loading?"not-allowed":"pointer",marginTop:2}}>
              {loading?"Entrando...":"→ Entrar"}
            </button>
            <div style={{textAlign:"center"}}>
              <button onClick={forgotPassword} style={{background:"none",border:"none",color:"#64748b",fontSize:12,cursor:"pointer",fontFamily:"inherit",textDecoration:"underline"}}>Esqueci minha senha</button>
            </div>
          </div>
        )}
      </div>
