...
function exportSummaryExcel(txns, entityType, company, businessDesc) {
  ...
  rows.push(["","",""],["";"",company||""],["";"",businessDesc||""],["";"",`${cfg.label} (${cfg.form})`],["";"",`Generated by OneTouch Tax ${APP_VERSION}`]);
...
function exportPDF(txns,entityType,userName,company){
  ...
  const rows=txns.map(t=>{const type=getCatType(t.aiCategory,entityType);const tl=type==="nondeduc"?"Non-Ded.":type==="income"?"Income":"Expense";return `<tr><td>${t.date}</td><td>${t.description}</td><td style="text-align:right;color:${t.amount>=0?"#10b981":"#334155'}">${t.amount>=0?"+":"-"}$${Math.abs(t.amount).toFixed(2)}</td><td>${t.aiCategory||"\u2014"}</td><td>${tl}</td><td style="color:${t.status==="approved"?"#10b981":t.status==="rejected"?"#ef4444":"#f59e0b"}">${t.status}</td></tr>`;}).join("");
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        ...
        {ccTxns.length>0&&(
          <div style={{display:"flex",gap:8}}>
            {pending>0&&<button onClick={approveAll} style={{background:"#f0fdf4",color:"#15803d",border:"1px solid #86efac",borderRadius:10,padding:"8px 16px",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>&check; Confirmar todas ({pending})</button>}
            ...
            return(
              <div key={t.id} style={{display:"grid",gridTemplateColumns:"85px 1fr 110px 175px 75px 130px",gap:8,alignItems:"center",padding:"11px 14px",borderBottom:"1px solid #f8fafc",background:t.status==="approved"?"#fdf4ff":t.status==="rejected"?"#fef2f2":"#fff"}}>
                ...
                <div style={{display:"flex",gap:3,justifyContent:"center",flexWrap:"wrap"}}>
                  {isEd?<><button onClick={()=>saveEdit(t.id)} style={{background:"#f0fdf4",color:"#15803d",border:"none",borderRadius:7,padding:"4px 8px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>&check;</button><button onClick={()=>setEditId(null)} style={{background:"#fef2f2",color:"#dc2626",border:"none",borderRadius:7,padding:"4px 8px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>\u2715</button></>
                  :t.status==="approved"?<><span style={{fontSize:10,fontWeight:700,color:"#7e22ce"}}>&check; Ok</span><button onClick={()=>{setEditId(t.id);setEditCat(t.aiCategory||"");}} style={{background:"#fdf4ff",color:"#7e22ce",border:"none",borderRadius:7,padding:"3px 7px",fontSize:10,cursor:"pointer",fontFamily:"inherit"}}>\u270E</button></>
        ...
        {tab==="transactions"&&<div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              {txns.filter(t=>t.status==="pending").length>0&&<button onClick={approveAll} style={{background:"#f0fdf4",color:"#15803d",border:"1px solid #86efac",borderRadius:10,padding:"7px 14px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>&check; Confirmar todas ({txns.filter(t=>t.status==="pending").length})</button>}
